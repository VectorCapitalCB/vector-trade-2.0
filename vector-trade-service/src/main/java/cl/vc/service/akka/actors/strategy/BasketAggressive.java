...
public class BasketAggressive implements StrategyI {
    ...
    public BasketAggressive(RoutingMessage.Order order, String idSubscribe, Logger fileLog, ActorRef actorGroupPerOrder, ActorRef actorStrategy) {
    ...
    public void onStatistic(MarketDataMessage.Statistic statistic) {
        ...
        try {
            ...
            if (order.getLimit() <= 0) {
                ...
                blockOrders = true;
                RoutingMessage.Order order1 = order.toBuilder().setText("Best Aggressive!!!! Limit must not be Zero")
            ...
            if (order.getOrderQty() <= 0) {
                ...
                RoutingMessage.Order order1 = order.toBuilder()
                        .setText("Basket Aggressive!!!! qty must not be Zero")
            ...
            if (limit_calculate_buy <= 0) {
                limit_calculate_buy = statistic.getLast() * (1 + order.getLimit() / 100d); //50000
            }

            if (limit_calculate_sell <= 0) {
                limit_calculate_sell = statistic.getLast() * (1 - order.getLimit() / 100d); //40000
            }
            ...
            if (order.getSide().equals(RoutingMessage.Side.BUY)) {
                ...
                if (!blockOrders && order.getOrdStatus().equals(RoutingMessage.OrderStatus.PENDING_NEW)) {
                    if (limit_calculate_buy >= statistic.getAskPx()) {
                        blockOrders = true;
                    ...
                    if (limit_calculate_buy >= statistic.getAskPx()) {
                        ...
                        double px = bookSnapshot.getStatistic().getAskPx();
                        px = Ticks.applyRulePrice(order.getSecurityExchange(), px);
                ...
                if (!blockOrders && order.getOrdStatus().equals(RoutingMessage.OrderStatus.PENDING_NEW)) {
                    ...
                    if (limit_calculate_sell <= statistic.getBidPx()) {
                        blockOrders = true;
                    ...
                    if (limit_calculate_sell >= statistic.getBidPx()) {
                        ...
                        double px = bookSnapshot.getStatistic().getBidPx();
                        px = Ticks.applyRulePrice(order.getSecurityExchange(), px);
    ...
    @Override
    public void onIncrementalBook(MarketDataMessage.IncrementalBook inc) {
